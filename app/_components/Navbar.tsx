...
const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        <button
          className={`md:hidden p-2 rounded-lg border transition-colors duration-200 ${
            isMenuOpen
              ? "border-gray-300/70 bg-white/80 text-gray-900"
              : "border-gray-200/60 bg-white/60 text-gray-700 hover:bg-white/80"
          } backdrop-blur`}
      ...
      {/* Mobile Dropdown */}
      {isMenuOpen && (
        <div className="md:hidden container mx-auto px-6 mt-2">
          <div className="p-[1px] rounded-2xl bg-gradient-to-r from-cyan-400/60 via-white/60 to-purple-500/60 shadow-xl shadow-cyan-500/10">
            <div className="rounded-2xl border border-white/70 bg-white/80 backdrop-blur-xl px-3 py-3 transition-all duration-300 ease-out">
              <div className="flex items-center justify-center pb-2">
                <span
                  className="h-1 w-10 rounded-full bg-gray-300/80"
                  aria-hidden="true"
                />
              </div>
              <div className="flex flex-col divide-y divide-gray-200/70">
                {menuItems.map((item, index) => (
                  <Link
                    ...
                    href={item.href}
                    onClick={(e) => handleSmoothScroll(e, item.href)}
                    className="px-3 py-3 text-base font-medium text-gray-800 hover:text-gray-900 rounded-lg hover:bg-gray-100/70 transition-colors"
                  >
                    {item.label}
                  </Link>
                ))}
